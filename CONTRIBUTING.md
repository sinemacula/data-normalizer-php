...
```bash
git clone git@github.com:sinemacula/data-normalizer-php.git
cd data-normalizer-php
composer install
```
...
```text
feat: add IBAN normalizer
fix: preserve redacted SSNs through the digit strip
test: cover the fuzzy country-match boundary
...
```bash
vendor/bin/phpunit tests/Unit/Types/EmailTest.php
vendor/bin/phpunit --filter testNormalizesValueToExpectedResult tests/Unit/Types/EmailTest.php
```
